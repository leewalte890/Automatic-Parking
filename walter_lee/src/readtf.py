...
class get_cam_tf:
    ...
    def wait(self):
        rate = rospy.Rate(4)
        if not self.trans:
            rospy.loginfo('Waiting for base_footprint and camera_rgb_optical_frame')
        while not self.trans:
            rate.sleep()

if __name__ == '__main__':    
    ''' Example node to read in transformation from base_footprint to camera_rgb_optical_frame'''
